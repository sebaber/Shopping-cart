...
function booksReducers() {
  ...
  switch (action.type) {
    ...
    case "UPDATE_BOOK":
      //Create a copy of current array of books
      var currentUpdateBooks = [].concat(_toConsumableArray(state.books));
      // Determine at wich index in books is the book to be deleted
      var indexToUpdate = currentUpdateBooks.findIndex(function (book) {
        return book.id === action.payload.id;
      });
      //Create book update
      var newBookUpdated = _extends({}, currentUpdateBooks[indexToUpdate], {
        title: action.payload.title
      });
      //use slice to remove the book
      return {
        books: [].concat(_toConsumableArray(currentUpdateBooks.slice(0, indexToUpdate)), [newBookUpdated], _toConsumableArray(currentUpdateBooks.slice(indexToUpdate + 1)))
      };
  };
  return state;
};
